...
numerator   = (r_ss / (A_ss * sp.alpha_K))^(sp.rho / (1 - sp.rho)) - sp.alpha_K;
denominator = sp.alpha_L * sp.Lbar^sp.rho + sp.alpha_E * E_ss^sp.rho;
K_ss = (numerator / denominator)^(-1 / sp.rho);

I_ss = sp.delta * K_ss;
Y_ss = A_ss * (sp.alpha_K*K_ss^sp.rho + sp.alpha_L*sp.Lbar^sp.rho + sp.alpha_E*E_ss^sp.rho)^(1/sp.rho);

% Total storage cost in SS (quadratic, commodity-specific):
%   SC_c = kappa_1_c * R_c_bar + kappa_2_c * R_c_bar^2

r_out = r_ss;
w_ss  = A_ss * sp.alpha_L * (sp.Lbar^(sp.rho-1)) * ...
        (sp.alpha_K*K_ss^sp.rho + sp.alpha_L*sp.Lbar^sp.rho + sp.alpha_E*E_ss^sp.rho)^(1/sp.rho - 1);
pE_ss = A_ss * sp.alpha_E * (E_ss^(sp.rho-1)) * ...
        (sp.alpha_K*K_ss^sp.rho + sp.alpha_L*sp.Lbar^sp.rho + sp.alpha_E*E_ss^sp.rho)^(1/sp.rho - 1);

total_storage_cost_ss = 0;
total_purchase_cost_ss = 0;
for ci_tmp = 1:N_COMM
    c_tmp = COMM{ci_tmp};
    R_c_bar_tmp = sp.(['R_' c_tmp '_bar']);
    k1_c = sp.(['kappa_1_' c_tmp]);
    k2_c = sp.(['kappa_2_' c_tmp]);
    E_D_c_import_tmp = sp.(['ED_' c_tmp '_bar']);
    total_storage_cost_ss = total_storage_cost_ss + ...
        k1_c * R_c_bar_tmp + k2_c * R_c_bar_tmp^2;
    total_purchase_cost_ss = total_purchase_cost_ss + pE_ss * E_D_c_import_tmp;
end

% Resource constraint in SS:
%   Y = C + I + phi_IQ*Y + purchase_cost + storage_cost  (ToT = 0)
% => C = (1 - phi_IQ)*Y - I - purchase_cost - storage_cost
C_ss = (1 - sp.phi_IQ) * Y_ss - I_ss - total_purchase_cost_ss - total_storage_cost_ss;
...
ys(1:n_core) = [A_ss; Y_ss; C_ss; I_ss; K_ss; r_out; w_ss; pE_ss; E_D_service_ss; E_out; NX_ss];
