...
% Steady-state solver for ToyModelSOEMC (multi-commodity SOE with strategic reserves)
%
% KEY FEATURES:
%   - Strategic reserve capacity constraints affecting energy services
%   - Carrier-specific storage costs (oil and coal)
%   - Energy efficiency parameter g_E available for future use
%
...
%   Euler: 1/C = beta * (1/C(+1)) * (r + 1 - delta)
%   In steady state: 1 = beta * (r_ss + 1 - delta)
%   => r_ss = 1/beta + delta - 1
...
% Total storage cost in SS (carrier-specific quadratic cost):
%   SC_c = kappa_1_c * R_c_bar + kappa_2_c * R_c_bar^2
%   kappa_1_c = delta_R * p_D_bar (same for all carriers)
%   kappa_2_c: carrier-specific infra cost (oil: 0.068, coal: 0.172)
total_storage_cost_ss = 0;
for ci_tmp = 1:N_COMM
    c_tmp = COMM{ci_tmp};
    R_c_bar_tmp  = sp.(['R_' c_tmp '_bar']);
    kappa_1_c    = sp.(['kappa_1_' c_tmp]);
    kappa_2_c    = sp.(['kappa_2_' c_tmp]);
    total_storage_cost_ss = total_storage_cost_ss + ...
        kappa_1_c * R_c_bar_tmp + kappa_2_c * R_c_bar_tmp^2;
end
...
% ----------------------------------------------------------------
% PART 2: Energy efficiency (BGP component)
%   g_E drives NEMP energy efficiency gains: renewable transition,
%   grid modernization, demand-side efficiency programs.
% ----------------------------------------------------------------
g_E = sp.g_E;   % Energy efficiency growth rate (1.5% p.a. default)

% ----------------------------------------------------------------
% PART 2a: Capacity steady state — compute Q_c_bar and store in params
